...
with st.sidebar:
    ...
    with colA:
        ...
        target_age  = st.number_input("目標年齢", 40, 90, 60, 1)
    with colB:
        initial_assets = st.number_input("現在の金融資産（万円）", 0, 999999, 100, 10)

    # 年収（額面）推移
    st.subheader("額面年収の推移")
    ...
    # 貯蓄と投資
    st.subheader("貯蓄と投資")
    mode = st.radio("貯蓄方法", ["割合で指定", "固定額で指定"], horizontal=False, index=1)
    if mode == "割合で指定":
        ...
        with colp3:
            save_amt_peak = st.number_input("教育費ピークの貯蓄額（万円/年）", 0, 100000, 100, 10)
...
st.metric(f"🏡 {target_age}歳時点の不動産純資産（万円）", f"{(df.iloc[-1]['土地価値（万円）'] + df.iloc[-1]['建物価値（万円）'] - df.iloc[-1]['住宅ローン残高（万円）']):,}")
